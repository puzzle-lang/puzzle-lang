...
fun parseModifiers(cursor: PzlTokenCursor): List<Modifier> {
	...
	while (true) {
		modifiers += ModifierParser.of(cursor).parse() ?: break
	}
	checkModifierOrder(cursor, modifiers)
	return modifiers
}

private class ModifierParser private constructor(
	private val cursor: PzlTokenCursor,
) : PzlParser {
	
	companion object : PzlParserProvider<ModifierParser>(::ModifierParser)
